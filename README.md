...
- ✅ 章節滿意度投票系統（表情符號投票）
- ✅ 社群媒體分享功能（X、Facebook、Line、Threads）
...
### 首頁UI重構
- ✅ 移除頁面標題區塊（「最新章節」和「跟隨故事的最新發展」文字）
- ✅ 章節標題顯示格式優化：`章節編號 章節標題`（移除「第x章：」前綴）
- ✅ 投票狀態標籤移至章節標題右邊
- ✅ 文章預覽功能：限制顯示前200字，支援點擊展開完整內容
- ✅ 跨故事章節排序：所有章節按生成時間由新到舊排序
- ✅ 新章節自動置頂功能
- ✅ 故事篩選功能：點擊故事標題篩選該故事所有章節
- ✅ 三按鈕導航系統：上一章、章節列表、下一章
- ✅ 投票項目收合功能：預設收合狀態，點擊展開投票選項
- ✅ 導航按鈕圖標化：移除文字，使用圖標按鈕
- ✅ 章節列表展開功能：顯示該故事所有章節並支援跳轉
- ✅ 簡化投票區塊：移除「章節投票」標題，用簡短文字說明投票狀態
- ✅ 移除故事起源投票區塊
- ✅ 修復投票重複計票問題（資料庫觸發器重複更新）
...
SOCIAL_SHARE_PLATFORMS=X、Facebook、Line、Threads
...
│               ├── {chapter_number}.webp  # 章節插圖（WebP 格式）
│               └── shares/    # 分享用圖片目錄
│                   ├── {chapter_number}_x.webp
│                   ├── {chapter_number}_facebook.webp
│                   ├── {chapter_number}_line.webp
│                   └── {chapter_number}_threads.webp
│
...
│   │   ├── Header.tsx          # 現有元件
│   │   ├── Footer.tsx
│   │   ├── Sidebar.tsx         # 電腦版側邊欄（5個主要選項）
│   │   ├── BottomNav.tsx       # 手機版底部導航（5個核心功能）
│   │   ├── MobileNav.tsx       # 手機版頂部導航
│   │   ├── RightSidebar.tsx
...
- **生成優化**：確保原本插圖先生成完成，再考慮分享圖片生成
- **標題格式**：章節標題不包含「第x章：」前綴，前端顯示為「章節編號 章節標題」
...
4. **插圖處理與儲存**：
   ...
   - **檔案命名**：使用章節編號作為檔名，確保唯一性
   - **本地儲存**：轉換後存到 `public/images/stories/{story_id}/{chapter_number}.webp`
...
| `story_id` | `UUID` | **外鍵**：指向所屬的故事 | 確保投票歸屬 |
| `voter_ip` | `VARCHAR(45)` | **投票者 IP 地址** | 用於防重複投票和冷卻時間判斷 |
| `voter_session` | `VARCHAR(255)` | **投票者會話 ID** | 追蹤投票行為 |
| `option_id` | `VARCHAR(10)` | **投票選項** | 'A', 'B', 'C' |
| `voted_at` | `TIMESTAMP WITH TIME ZONE` | **投票時間** | 用於冷卻時間計算（同一IP需間隔24小時） |
| `user_agent` | `TEXT` | **瀏覽器資訊** | 可選，用於分析 |
...
| `platform` | `VARCHAR(20)` | **分享平台** | 'x', 'facebook', 'line', 'threads' |
...
##### 分享平台支援
- **X**：1200x675px，280 字元限制
- **Facebook**：1200x630px，無字元限制
- **Line**：800x600px，無字元限制
- **Threads**：1080x1080px，2200 字元限制
...
const shareTemplates = {
  x: "📖《{storyTitle}》{chapterTitle}！{emoji} {hashtags}", // ~60 字元
...
- **檔案命名**：`{chapter_number}_{platform}.webp`
...
interface ShareRequest {
  platform: 'x' | 'facebook' | 'line' | 'threads';
}
...
4. **防作弊機制**：
   - **IP 冷卻限制**：同一 IP 地址需間隔 24 小時才能再次投票
   - **時間限制**：投票必須在有效期內進行
   - **選項驗證**：確保投票選項有效（A、B、C）
   - **重複投票檢查**：防止同一用戶重複投票
...
- `voting_status` 欄位追蹤投票狀態：
  - `'投票中'`：章節正在接受讀者投票
  - `'已截止'`：投票已達標並已生成新的章節，投票選項已鎖定

##### 投票狀態轉換規則

1. **初始狀態**：新章節建立時為 `'投票中'`
2. **達標觸發**：任一選項達到門檻票數時，立即變為 `'已截止'` 並觸發 AI 生成
3. **記錄結果**：投票狀態變為 `'已截止'` 後，將得票最高的選項記錄到 `user_choice` 欄位
4. **新章節置頂**：AI 生成新章節後，新章節立即置頂顯示給讀者

##### 投票選項顯示規則

- **投票中**：顯示所有選項，用戶可選擇投票（需檢查 IP 冷卻時間）
- **已截止**：顯示所有選項，得票最高的選項有明顯反白標示
...
#### 手機版底部導航欄（5個核心功能）
1. **🏠 首頁** - 導向主內容列表，最明顯的高亮狀態
2. **✨ 起源** - 導向故事起源投票頁面，具備 AI 創作科技感
3. **🔖 暫存** - 基於 Local Storage 的非會員收藏功能
4. **⚖️ 免責聲明** - 導向免責聲明和隱私權政策頁面
5. **🌐 語系選單** - 點擊彈出語言選項，支援繁體中文、簡體中文、English、日本語、한국어、泰文

#### 電腦版側邊欄（5個主要選項）
1. **🏠 首頁** - 保持現有功能
2. **✨ 起源** - 保持現有功能，圖示與手機版統一
3. **🔖 暫存** - 保持現有功能
4. **⚖️ 免責聲明** - 導向免責聲明和隱私權政策頁面
5. **🌐 語系選單** - 側邊欄底部下拉選單，支援繁體中文、簡體中文、English、日本語、한국어、泰文

#### 語系選單雙重實作策略
- **電腦版**：側邊欄底部保留下拉選單
- **手機版**：底部導航右邊增加語系彈窗選項
- **統一體驗**：兩種實作方式提供一致的語言切換功能
