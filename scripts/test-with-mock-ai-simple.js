...
/**
 * 使用模擬 AI 生成測試完整流程腳本（簡化版）
 * 使用方法: node scripts/test-with-mock-ai-simple.js
 */

const { Pool } = require('pg');
require('dotenv').config({ path: '.env.local', override: true });

async function testWithMockAISimple() {
  console.log('🤖 使用模擬 AI 生成測試完整流程（簡化版）...');
  ...
  try {
    ...
    `, [
      ...
      '002',
      '第二章：模擬 AI 生成的章節',
      '這是模擬 AI 生成的章節內容。主角選擇了選項A，勇敢地前進。在新的章節中，主角遇到了新的挑戰和機遇，故事繼續發展...',
      '主角勇敢前進，遇到新挑戰',
      '進行中',
...
// 執行模擬測試
testWithMockAISimple().catch(console.error);
